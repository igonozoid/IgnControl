<div class="max-w-5xl mx-auto py-5 px-4 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between mb-4">
        <h1 class="flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-neutral-100"><x-icon name="chart" class="w-4 h-4" />Fluxo de Caixa</h1>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="bg-white dark:bg-neutral-800 shadow-sm rounded-lg p-4">
            <p class="text-xs text-gray-500 dark:text-neutral-400">Saldo inicial</p>
            <p class="text-lg font-semibold text-gray-900 dark:text-neutral-100">{{ number_format($openingBalance, 2, ',', '.') }}</p>
        </div>
        <div class="bg-white dark:bg-neutral-800 shadow-sm rounded-lg p-4">
            <p class="text-xs text-gray-500 dark:text-neutral-400">Saldo final</p>
            <p @class([
        ...
        <table class="w-full text-xs">
            <thead class="bg-gray-50 dark:bg-neutral-700/50 text-xs text-gray-500 dark:text-neutral-400 uppercase">
                <tr>
                    ...
                    <th class="text-right px-4 py-2">Entradas</th>
                    <th class="text-right px-4 py-2">Saídas</th>
            ...
            <tbody>
                @forelse ($days as $day)
                    <tr class="border-t border-gray-50 dark:border-neutral-700">
                        ...
                        <td class="px-4 py-1.5 text-right font-medium text-gray-900 dark:text-neutral-100">{{ number_format($day['balance'], 2, ',', '.') }}</td>
                    </tr>
                @empty
