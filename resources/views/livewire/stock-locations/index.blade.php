<div class="max-w-4xl mx-auto py-5 px-4 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between mb-4">
        <h1 class="flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-neutral-100"><x-icon name="bank" class="w-4 h-4" />Locais de Estoque</h1>
        @if ($this->canWrite)
    ...
    <x-slide-over show="showForm" close="cancel" title="{{ $editingId ? 'Editar local' : 'Novo local' }}">
        <form wire:submit="save" class="space-y-3 text-xs">
            ...
            <div>
                ...
                <select wire:model="location_type" class="mt-1 block w-full rounded-md text-xs border-gray-300 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-100 shadow-sm">
                    @foreach (\App\Models\StockLocation::TYPES as $value => $label)
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                @forelse ($locations as $location)
                    <tr wire:key="stock-location-{{ $location->id }}">
                        <td class="px-4 py-2 text-xs text-gray-900 dark:text-neutral-100">{{ $location->name }}</td>
                        <td class="px-4 py-2 text-xs text-gray-500 dark:text-neutral-400">{{ \App\Models\StockLocation::TYPES[$location->location_type] ?? $location->location_type }}</td>
                        ...
                        @if ($this->canWrite)
                            <td class="px-4 py-2 text-right text-xs space-x-2 whitespace-nowrap">
                                <button wire:click="edit({{ $location->id }})" title="Editar" class="inline-flex text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300"><x-icon name="pencil" /></button>
                                <button wire:click="delete({{ $location->id }})" wire:confirm="Tem certeza que quer excluir este local?" title="Excluir" class="inline-flex text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300"><x-icon name="trash" /></button>
                            </td>
                        @endif
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-6 text-center text-xs text-gray-500 dark:text-neutral-400">Nenhum local cadastrado ainda.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
