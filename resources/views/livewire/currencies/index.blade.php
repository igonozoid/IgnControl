<div class="max-w-4xl mx-auto py-5 px-4 sm:px-6 lg:px-8">
    ...
    <x-slide-over show="showForm" close="cancel" title="{{ $editingCode ? 'Editar moeda' : 'Nova moeda' }}">
        <form wire:submit="save" class="space-y-3 text-xs">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block font-medium text-gray-700 dark:text-neutral-300">Código (ISO 4217)</label>
                    <input type="text" wire:model="code" maxlength="3" @if($editingCode) disabled @endif
                        class="mt-1 block w-full rounded-md text-xs border-gray-300 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-100 shadow-sm uppercase disabled:opacity-60">
                    @error('code') <span class="text-red-600 dark:text-red-400">{{ $message }}</span> @enderror
                    @if ($editingCode)
                        <p class="text-gray-400 dark:text-neutral-500 mt-1">Não dá pra mudar o código de uma moeda já cadastrada.</p>
                    @endif
                </div>
                <div>
                    <label class="block font-medium text-gray-700 dark:text-neutral-300">Símbolo</label>
                    <input type="text" wire:model="symbol" class="mt-1 block w-full rounded-md text-xs border-gray-300 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-100 shadow-sm">
                    @error('symbol') <span class="text-red-600 dark:text-red-400">{{ $message }}</span> @enderror
                </div>
            </div>
            ...
            <div>
                <label class="block font-medium text-gray-700 dark:text-neutral-300">Casas decimais</label>
                <input type="number" min="0" max="6" wire:model="decimals" class="mt-1 block w-32 rounded-md text-xs border-gray-300 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-100 shadow-sm">
                @error('decimals') <span class="text-red-600 dark:text-red-400">{{ $message }}</span> @enderror
            </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
            <thead class="bg-gray-50 dark:bg-neutral-700/50">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-neutral-400 uppercase">Símbolo</th>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-neutral-400 uppercase">Decimais</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                @forelse ($currencies as $currency)
                    <tr wire:key="currency-{{ $currency->code }}">
                        <td class="px-4 py-2 text-xs font-medium text-gray-900 dark:text-neutral-100">{{ $currency->code }}</td>
                        <td class="px-4 py-2 text-xs text-gray-700 dark:text-neutral-300">{{ $currency->name }}</td>
                        <td class="px-4 py-2 text-xs text-gray-500 dark:text-neutral-400">{{ $currency->symbol }}</td>
                        <td class="px-4 py-2 text-xs text-gray-500 dark:text-neutral-400">{{ $currency->decimals }}</td>
                        ...
                        @if ($this->canWrite)
                            <td class="px-4 py-2 text-right text-xs space-x-2 whitespace-nowrap">
                                <button wire:click="edit('{{ $currency->code }}')" title="Editar" class="inline-flex text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300"><x-icon name="pencil" /></button>
                                <button wire:click="delete('{{ $currency->code }}')" wire:confirm="Tem certeza que quer excluir esta moeda?" title="Excluir" class="inline-flex text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300"><x-icon name="trash" /></button>
                            </td>
                        @endif
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-6 text-center text-xs text-gray-500 dark:text-neutral-400">Nenhuma moeda cadastrada ainda.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
