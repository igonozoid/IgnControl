    ...
    <x-slide-over show="showForm" close="cancel" title="{{ $editingId ? 'Editar centro de custo' : 'Novo centro de custo' }}">
        <form wire:submit="save" class="space-y-3 text-xs">
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" wire:model="is_active" id="is_active" class="rounded border-gray-300 dark:border-neutral-600 text-indigo-600">
                    ...
                    <tr wire:key="cost-center-{{ $costCenter->id }}">
                        ...
                        @if ($this->canWrite)
                            <td class="px-4 py-2 text-right text-xs space-x-2 whitespace-nowrap">
                                <button wire:click="edit({{ $costCenter->id }})" title="Editar" class="inline-flex text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300"><x-icon name="pencil" /></button>
